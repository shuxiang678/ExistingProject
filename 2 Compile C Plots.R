...
treelist <- fread("H:/Shared drives/Model Comparison/Compilation_Code_R/PGYI Compiled/tree_list1.csv")
regen <- fread("H:/Shared drives/Model Comparison/Compilation_Code_R/PGYI Compiled/i_regeneration.csv")
...
plot2 <- plot %>%
  ...
  summarise(
    ...
    sphD91 = sum(sphD91),
    ba = sum(ba),
   # vol_0000 = sum(vol_0000),
    vol_1307 = sum(vol_1307),
   # vol_1510 = sum(vol_1510),
    biomass = sum(biomass),
    carbon = sum(carbon),
    .groups = "drop"
  )
...
fwrite(plot_level1, "H:/Shared drives/Model Comparison/Compilation_Code_R/PGYI Compiled/i_plot_level1.csv")

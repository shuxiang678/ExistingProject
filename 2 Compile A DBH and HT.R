...
trees_merge <- fread("H:/Shared drives/Model Comparison/Compilation_Code_R/PGYI Compiled/i_trees_merge.csv")
plot_measurement <- fread("H:/Shared drives/Model Comparison/Compilation_Code_R/PGYI Compiled/i_plot_measurement.csv")
...
trees <- left_join(trees[,!"species"],fread("H:/Shared drives/Model Comparison/Compilation_Code_R/GYPSY data/lookup/species.csv"),by="species_og") # Joines species and species group from a lookup table
...
temp <- left_join(trees,fread("H:/Shared drives/Model Comparison/Compilation_Code_R/GYPSY data/lookup/ht_to_dbh.csv"),by="species")
...
rm(plot);gc()
temp <- left_join(trees,fread("H:/Shared drives/Model Comparison/Compilation_Code_R/GYPSY data/lookup/dbh_to_ht.csv"),by=c("natural_subregion","species"))
...
fwrite(trees,"H:/Shared drives/Model Comparison/Compilation_Code_R/PGYI Compiled/i_trees_measurement_filled.csv")
...
# check outputs, output does not match
data2 <-  read.csv("H:/Shared drives/Growth & Yield Lab/Data Sets/PGYI/2023-09-08 PGYI Compiled/2_interim/trees_measurement_filled.csv")

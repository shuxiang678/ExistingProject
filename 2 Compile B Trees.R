...
trees <- fread("H:/Shared drives/Model Comparison/Compilation_Code_R/PGYI Compiled/i_trees_measurement_filled.csv")
...
trees.tap <- left_join(trees,fread("H:/Shared drives/Model Comparison/Compilation_Code_R/GYPSY data/lookup/natsub.csv"),by="natural_subregion")

trees.tap$natsub[trees$species%in%c("BW","FD","LT","PJ","SE")] <- 0

trees.tap <- left_join(trees.tap,fread("H:/Shared drives/Model Comparison/Compilation_Code_R/GYPSY data/lookup/taper.csv"),by=c("species","natsub"));gc()
...
temp <- trees.tap[!is.na(trees.tap$height) & trees.tap$height>=1.3 & !is.na(trees.tap$dbh) & topdibM<trees.tap$dbh,
...
tree_list1 <- trees.bio %>%
  ...
  select(company, company_plot_number, measurement_number, measurement_year, tree_number, 
         ...
         severity3, ht_stat, dbh_stat, ba, sph, vol_1307, biomass, carbon, spp_grp, condec) #need to add tvol 

fwrite(tree_list1,"H:/Shared drives/Model Comparison/Compilation_Code_R/PGYI Compiled/tree_list1.csv") 
...
fwrite(tree_ages,"GYPSY data/intermediate/i_tree_list1.csv")
